/*conf抜いたLaplacianを読み込み、ジャックナイフ平均と誤差だす。*/
/*information 
...
int call_data(int b,int it,double local[binnumber*datasize]){
	char fname[200]={0};
	sprintf(fname,"%s/Laplacian/binLaplacian/xyz/Lap.%06d-%06d.%s.it%02d",in_dir_path.c_str(),binnumber,b,base,it);
...
void out_data(int it,double avesub[datasize],double errsub[datasize]){
	...
	char fnamer[200]={0};
	char fnamexyz[200]={0};
	sprintf(fnamer,"%s/Lap.%06d.%s.it%02d",r_out_dir_path.c_str(),binnumber,base,it);
	sprintf(fnamexyz,"%s/Lap.%06d.%s.it%02d",xyz_out_dir_path.c_str(),binnumber,base,it);
