...
int main(int argc,char** argv){
	...
	for ( int j=0; j<binnumber; j++) {
		...
		for (int it=T_in; it<T_fi+1; it++) {
			...
			divide(&(Tdep_BSwave[0]),&(sub_BSwave[0]));
		for (int it=T_in; it<T_fi+1; it++) {
...
void calc_Tdep(COMPLEX sub_BSwave[XYZnodeSites*TnodeSites],COMPLEX Tdep_BSwave[XYZnodeSites*TnodeSites]){
	...
	for (int y=0; y<YnodeSites; y++) {
	for (int x=0; x<XnodeSites; x++) {
		Tdep_BSwave(x,y,z,it)=(sub_BSwave(x,y,z,it-1)-sub_BSwave(x,y,z,it+1))/2.0;
	}}}
}			
}
/*******************************************************************/
//BS波動関数で割る　1/ψ*dψ/dtを行う
/*******************************************************************/
void divide(COMPLEX Tdep_BSwave[XYZnodeSites*TnodeSites],COMPLEX sub_BSwave[XYZnodeSites*TnodeSites]){
	for (int id=0; id<XYZnodeSites*TnodeSites; id++) {
		Tdep_BSwave[id]=Tdep_BSwave[id]/sub_BSwave[id];
	}
}
