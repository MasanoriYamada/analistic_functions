...
template<typename DATA> void IODATA::callData(DATA in ,std::string inPath ,std::string staticInfo ,std::string physInfo ,int iconf ,int it){
  checkConfsize();
  if(r_bSwich)sprintf(inFileName,"%s/%s.%s.%06d-%06d.it%02d",inPath.c_str(),staticInfo.c_str(),physInfo.c_str(),binnumber,iconf,it);
  else if(!r_bSwich)sprintf(inFileName,"%s/%s.%s.%06d-%06d.it%02d",inPath.c_str(),staticInfo.c_str(),physInfo.c_str(),binnumber,iconf,it);
